...
int VulkanEngine::rateDeviceSuitability(VkPhysicalDevice device) {
    ...
    if (!indices.isComplete() || !extensionsSupported || !deviceFeatures.geometryShader || !swapChainAdequate)
        return 0;
...
void VulkanEngine::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    for (auto object = objects->begin(); object != objects->end(); object++) {
        ...
        for (int i = 0; i < object_indices.size(); i++) {
            indices.push_back(object_indices[i] * index);
        }
        index++;
    }
